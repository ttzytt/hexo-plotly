...
const { optionalSiteUrl } = require('./config');
...
function loaderHtml(hexo, config, i18n, context, mathJaxEnabled) {
  ...
  const pageCustomStylesheet = optionalSiteUrl(
    hexo,
    context?.plotly_custom_stylesheet
  );
  const bootOptions = {
  ...
  if (config.urls.customStylesheet) {
    stylesheetLinks.push(
      `<link rel="stylesheet" href="${escapeHtml(config.urls.customStylesheet)}" data-hexo-plotly-custom-styles>`
    );
  }

  if (
    pageCustomStylesheet &&
    pageCustomStylesheet !== config.urls.customStylesheet
  ) {
    stylesheetLinks.push(
      `<link rel="stylesheet" href="${escapeHtml(pageCustomStylesheet)}" data-hexo-plotly-page-styles>`
    );
  }
